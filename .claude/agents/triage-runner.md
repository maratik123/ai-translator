...
- **Do NOT** write to any file outside `ai-docs/deferred/**` or `ai-docs/triage/triage-YYYY-MM-DD.progress.md` (this explicitly excludes `ai-docs/learnings.md`, `AGENTS.md`, `.claude/**`, source files, `Cargo.toml` / `Cargo.lock`). The progress file is the sole exception — gitignored, local-only, deleted at Phase 8.
