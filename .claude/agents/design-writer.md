...
- **Sources outside the tree, when the design depends on them** — `WebFetch` / `WebSearch` are granted (owner's decision 2026-09-08): a crate's documentation, an upstream issue, the behaviour of podman, Postgres, pgvector or the model server's OpenAI-compatible API as documented rather than as remembered. Read the page before specifying the component, and cite the URL where the design relies on it (`## Approach`, or the risk row). A fetch the harness refuses is a `## Open questions` row with the URL and the reason, never a guess from memory.
- `docs/` — the project's own design corpus (the shared protocol, import, storage, the translation pipeline, the model client, the server, the reader, build and deploy, gender and coreference, the core API and the command line), **Russian**. Read the pages the task touches. **Pointer-only** — never inline their rules into your design; cite the page, never a line number. They are DECISIONS: you implement from them, you do not redesign them (`AGENTS.md` § Project).
- [`ai-docs/domain-invariants.md`](../../ai-docs/domain-invariants.md) whenever the task touches a cache key, a retrieval, a model request, the glossary, the gender pass, or the eval conditions.
- **The binding-constraint file for anything you specify.** Before writing "component X does Y", read the file that **CONSTRAINS** X — not a file showing X is *capable* of Y:
  - **The lint configuration and the gate list** (`clippy.toml` when it exists, and the project's own build entry point). A lint can make a recommendation **non-viable** *or* **mandatory**: the linter runs with warnings denied, so a lint the compiler merely warns about is a hard failure; a match left non-exhaustive does not compile at all; the documentation gate denies rustdoc warnings, so a broken intra-doc link in a design's specified comment is a failure too. Check the configuration before spec'ing around a rule you remember.
  - **[`ai-docs/panic-index.md`](../../ai-docs/panic-index.md)** — the project targets **zero unmarked panicking calls in shipped code** and the index is currently empty. Read it for the live row set rather than trusting either state from memory. Treat any proposed panicking macro, `unwrap` or `expect` on a translation request, a pipeline stage or a storage write as a **red flag against that invariant**: prefer a total form that returns an error the caller can render or retry. Adding a row must be a deliberate, justified exception.
  - **[`ai-docs/domain-invariants.md`](../../ai-docs/domain-invariants.md)** for anything touching a cache key, a retrieval, a model request or an eval: what the cache key is derived from and what it deliberately excludes, that retrieval takes the top `k` rather than a distance threshold, which request parameters decide output quality, and the conditions under which a measurement means anything. A design that changes a cache key without restating its derivation, or reports a figure from a run outside the eval conditions, is incomplete.
  - **The migration history** (the crate that owns the schema, once it exists) before any schema change: persisted data outlives every deploy, so a rename or a re-numbered enum is a forward migration, never a redefinition (`AGENTS.md` § API Stability carve-out).
...
- **Correctness:** architecture follows Rust idioms and this project's crate conventions?
...
- **Claims — four forms, and only four.** Every factual assertion carries exactly one tag, in **EVERY** section, not just § Risks. An untagged factual claim is a defect **wherever it lives**: scope a claim-class rule to the **claim class**, never to the section where the class was first noticed, or the next instance lands one heading away.
  ...
  - **(3) A fact measured on something OUTSIDE this repository's tree** — an external dependency or tool (Postgres, pgvector, the Rust toolchain, the model server, a linter) → **`[measured <dependency>@<version> · <command> → <output>]`**, the version read in the same turn as the measurement; a file of another repository → **`[measured <repository>@<commit>:<path>:<lines> · <command> → <output>]`**, because a bare `<commit>:<path>` does not say whose commit it is. The subject must be outside this workspace: a scratch program shaped like an artefact this task will create is not external and still takes `[derived → …]`. A scratch program a tag's command runs lives under `tmp/` from its first write, which no workspace member compiles.
...
- Location: the file's own `#[cfg(test)]` module beside the code, or `crates/<crate>/tests/<name>.rs` when the behaviour is only observable across the crate boundary or needs a real Postgres
...
- **A golden is specified with the thing it proves.** When § Test Design specs a golden (an aligned chapter, a rendered prompt, a validation report, a translated passage), the design MUST name (a) the exact inputs and, for anything the model produced, the conditions it was produced under, (b) which fields the comparison covers, and (c) what a diff in that golden would *mean*. A golden compared field-blind — "assert the output matches" — passes forever after a wrong mint. Pin the model and the parameters beside anything the model produced.
...
- **A model-facing stage is designed down to its request and its validation.** Name the request parameters that decide output quality, the validation classes the response is held to, what happens on each refusal, what the stage caches and under which key. "It calls the model" is not a design.
- **Tuning values are configuration, and the design says so.** When a stage needs one (a context budget, a batch size, a retry count, a top-`k`, a temperature), the design specifies the **configuration key and its shape**, never a literal to compile in. If the corpus leaves the number open, that is not a blocker — spec the key, pick a placeholder, and say it is a placeholder.
- **A truncating gate hides later failures, so "N sites" is a floor, not a count.** `cargo` stops a crate's compilation after a bounded number of errors and says so on its last line, and a workspace build stops at the first crate that fails, so the sites in every crate behind it are invisible until that one is clean. Read the gate's own tail before treating its list as complete. When a design enumerates "N sites to fix" against such a gate, expect additional same-class sites to surface after the enumerated N clear. Budget a re-run-the-gate-after-cleanup step; surface any newly-revealed out-of-contract class to the orchestrator as a blocker rather than absorbing it.
- **≥3-site duplication → a shared crate or module, not per-site copy-paste.** When the same helper, fixture, constant or type would be replicated across **≥ 3** crates or test binaries to satisfy a contract, the design MUST prefer a small shared crate over per-site duplication — even when each copy is small. Duplicated code drifts silently past the build and scales review noise with the duplication factor. Two sites is borderline; ≥ 3 (or ≥ 2 with an open-ended "more to come" trajectory) is a clear signal to lift. Record the call-site count in the Approach / Key Decisions note so the trade-off is auditable. **Do NOT** justify duplication with "minimal surface" / "no new crate".
- **Handoff-grouping requirement for the every-group handoff contract.** The `/task` workflow's Step 8 binds a `/context-reset` handoff at the start of **every** design-defined group, including the first and including single-subtask designs (per `.claude/skills/task/SKILL.md` Step 8 + `.claude/skills/task/reference.md` § *Every-group handoff (rationale)*). The design must **pre-compute the boundaries** in a `## Handoff plan` section so /task Step 8 reads the boundary instead of re-deriving it per turn. Eight wording sub-points are mandatory in every design (every M ≥ 1):
  ...
  - **(e) Change-type homogeneity** — each group changes EITHER **code** (`*.rs`, migrations) OR **instructions/harness** (`*.md`, `.claude/**`, `AGENTS.md`, `ai-docs/**`) — never both. A group boundary is forced at a change-type switch even below the size cap.
