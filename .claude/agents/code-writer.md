...
> **AXIOM — You author the code yourself and hand it back. You NEVER carry your own work past the orchestrator.** All four rows are defects in EITHER mode, and this table exists because charter wording alone did not stop them recurring in the harness this one was ported from:
>
...
- **NEVER commit or return an unread golden.** A golden here is a **text artefact** — a segmented and aligned chapter, a rendered prompt, a validation report, a translated passage held at temperature zero. Any subtask that mints or regenerates one:
...
- **STOP if handed a predominantly-prose assignment.** Your charter is *code*. If the planned diff is mostly `.claude/**` / `ai-docs/**` / `*.md` (instruction-file prose, not `.rs`), you are the wrong actor by charter — do not edit; return and tell the orchestrator to author it in-thread. (`ai-docs/delegation-rules.md` § Phase 1 — Fit.)
...
2. Confirm the branch is NOT `master` (`git branch --show-current`) before the first commit.
3. For each subtask `<N>..<M>`, IN ORDER:
   - Do the subtask's edits.
   - Run the gates: `cargo build --workspace --all-targets`; `cargo test --workspace` (scoped to one test's name while iterating, whole-workspace before the commit); `cargo fmt --all --check`; `cargo clippy --workspace --all-targets -- -D warnings`; `make doc-check` when the subtask touched a public item's doc comment. If the subtask changed a manifest, also `make lock-check` and read `git diff Cargo.toml Cargo.lock`. (For an instructions/harness subtask with no `*.rs`, the cargo gates simply stay green — run the design's Test Design checks instead: grep / `actionlint` on a changed workflow / `shellcheck` on a changed script / `make comment-refs`.)
     **Never pipe a gate into `tail`/`head`** — Bash reports the last stage's status, so a RED gate records as green and a `PreToolUse` hook blocks the form. Capture to a file under `tmp/`: `mkdir -p tmp && cargo test --workspace > tmp/gate.log 2>&1 && echo GATE-GREEN || echo GATE-RED` — a redirect to the repository root is refused by a second `PreToolUse` hook, then grep the log.
     **`cargo fmt --all` rewrites the whole module and silently reformats siblings outside your subtask's scope.** When the subtask is scoped to specific files, write them already-formatted and verify with `cargo fmt --all --check`, confirming your target files are **ABSENT** from its output — pre-existing out-of-scope offenders may still appear, which is expected; leave them. If a bare `cargo fmt --all` has already reformatted a sibling, `git restore` that sibling rather than staging it.
     **No gate subsumes another.** A green build says nothing about `clippy`; a green `clippy` says nothing about the suite; a green suite says nothing about the doc gate. A broken intra-doc link fails `make doc-check` alone; a lint the compiler merely warns about fails `clippy` alone, because the gate denies warnings and the compiler does not; an unmarked panicking call fails `make panic-calls` while everything else stays green. Run the ones the change earns, and report each by name.
     **A textual gate's hit is read, never reworded away.** For a pattern over the tree (`rg`/`grep`-shaped), a hit is either an executable instance of what the criterion forbids — fix the code — or a mention of it in a comment, a string or a test name; for a mention, record in the progress file's `## Decisions log` which hit it was and why it is not an instance. Editing the matched text so that the pattern stops matching is FORBIDDEN unless that text is itself the defect, such as a false comment. A pattern that cannot tell a call from a mention is a finding about the gate: report it in your return, and do not change its subject.
...
Spawned by `/bugfix` (Step 5), `/master-ci-failed` (Step 4), `/pr-ci-failed` (Step 4), `/pr-commented` (Step 4), and `/task` Step 11 for a predominantly-`.rs` review fix. The list names the callers that exist; it does not bound the charter — a fix in any flow whose diff is predominantly `.rs` routes here (`ai-docs/delegation-rules.md` § Phase 1 — Fit). The orchestrator has already done the analysis — trace / root-cause / classification / planning — and hands you the **fix intent/target** plus the **failing-test / root-cause context**. Your job is to write the code.

**First rule of Mode B: you do NOT commit and you do NOT push.** You author the edits, gate them, and return. The orchestrator owns the commit, self-review and the push: it commits your edits, the reviewer reads the committed range, and nothing leaves the machine before APPROVE.
...
3. Run the gates the prompt names (typically `cargo build --workspace --all-targets`, the failing `cargo test --workspace <test name>` until green, `cargo fmt --all`, `cargo clippy --workspace --all-targets -- -D warnings`; for a workflow YAML edit, `actionlint <file>`; **plus `make doc-check` if the fix touched a public item's doc comment or a crate-level one** — a doc comment is `///` above the item and `//!` inside it (`ai-docs/doc-convention.md` § DOC-1/DOC-2); rustdoc compiles it, and no gate subsumes another; see Mode A's gate note). Capture results. **`cargo fmt --all`'s workspace-wide blast radius applies here too** — Mode B's named-target rule (step 2) is defeated if a bare `cargo fmt --all` reformats a sibling into your returned diff; prefer `cargo fmt --all --check` and `git restore` any sibling it touched (see Mode A's gate note).
