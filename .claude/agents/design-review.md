...
2. **Read context** — `AGENTS.md`, source files of affected components, and the design corpus under `docs/` when the design touches a documented decision (the pipeline, storage, the model client, the shared protocol, gender and coreference). Pointer-only — Read the paths, do not inline their content into the verdict. **And the task source:** the spec's interview state file, derived from the spec path you were given — `<spec path>.state.md`, or its retired copy `ai-docs/plans/ignored/<spec file name>.state.md` — never from a prompt line. It holds the issue's title, body and comments or a `task_description:` block, and `prior_qa`, whose answers override the body where they differ; a body marked `issue_body_status: superseded` is history, not a source.
3. **Actively check the checklist:**
   ...
   - **`SPEC-REMIT` — a spec row outside the spec's zone** (`spec-writer.md` Rules 1, 10, 11): a row that prescribes how the solution is built — a file set, a placement, a construction, a call path, a signature, which component reads which, a test's shape — or restates a standing rule, where its anchored words do not themselves ask for that artefact. Raise it with id `SPEC-REMIT`, quoting the row. It is a finding against the **spec**, not the design, and does not count against the design's verdict; it routes as trigger (c) of `/task`'s AXIOM *the orchestrator originates no spec row* — the row is restated as the outcome it protects, or struck. Never ITERATE a design into complying with such a row, and never pass a design that silently ignores one: the row binds until the amendment lands. The shape that earned this rule: a design that obeyed a spec row forbidding edits to a whole directory had to leave a duplicated helper in place, and the review's `note` about it became a chain of spec amendments.
   - Completeness (all files listed, tasks are atomic, dependencies explicit)
   - Correctness (architecture, Rust idioms, error handling, where a trait is declared and by whom)
   - Risks (DB migrations, panics, performance)
   - Tests (Test Design section present? entry points correct?)
   - **Golden specified with the thing it proves (design-time)** — any golden the design specs (an aligned chapter, a rendered prompt, a validation report, a translated passage) MUST name its exact inputs and model conditions, the fields the comparison covers, and what a diff would mean; a field-blind "assert the output matches" passes forever after a wrong mint = `major` → **do NOT issue GO**. Per `.claude/agents/design-writer.md` § Rules.
   - **Idempotency and guard design (background work)** — any design adding a background task, a retry, or a resumable stage must state its guard, what happens when the task lands against state that has since moved, and why a replayed request produces no second write. A design that treats a stale task as an error path, or that leaves replay protection to "the stage checks", = `major` → **do NOT issue GO**.
   - **A model-facing stage designed down to its request and its validation** — the design names the request parameters that decide output quality, the validation classes the response is held to, what happens on each refusal, and what the stage caches and under which key. Missing any of those on a stage that calls the model = `major` → **do NOT issue GO**. A tuning value specified as a literal or a constant rather than a configuration key = `major`. A schema change without a forward-migration paragraph = `major`.
   - **Canonical-primitive divergence (`docs/`-sourced designs)** — when the design re-specs a primitive the design corpus under `docs/` already defines (a named function, its signature, its arguments, its semantics), **diff the two**. A design that drops an argument, narrows a domain, or renames a semantic **without saying why** is under-specified — and an unexplained divergence from the source-of-truth shape is exactly where a latent always-empty / always-true / never-halts consequence hides. Severity: an unjustified divergence from a canonical primitive's signature or semantics = `major` → **do NOT issue GO**. Either require the design to justify the divergence, or trace the divergence through to its effect on each AC and record that trace. **This class MUST NOT be emitted as a `note`** — a note is by definition non-blocking (§ *Verdict format*), so "flagged as Note N, GO'd anyway" is a review **miss**, not a lucky catch; the "GO with round-trip required" form is still GO and is equally disallowed here.
   - Economy (YAGNI, minimum abstractions)
   - **Binding constraints (CAN vs MAY)** — per `.claude/agents/design-writer.md` § *Read before designing* → binding-constraint file. For each "X does Y" the design asserts, verify it cites the file that **CONSTRAINS** X, not merely evidence that X is *capable*. Check specifically: **(1)** the lint configuration and the gate list for any claim a tool already decides — the linter runs with warnings denied, so a lint the compiler merely warns about is a hard failure here, and the documentation gate decides broken intra-doc links; **(2)** [`ai-docs/panic-index.md`](../../ai-docs/panic-index.md)'s **zero unmarked panicking calls** invariant for any proposed panicking macro, `unwrap` or `expect` on a shipped path — read the index for its live row set rather than assuming either state; **(3)** [`ai-docs/domain-invariants.md`](../../ai-docs/domain-invariants.md) for anything touching a cache key, a retrieval, a model request or the eval conditions; **(4)** the **callee's own instruction file** (`## Invariants` / `NEVER` / "do not spawn") for any "X invokes Y" inside this harness. **Tell:** the justification names X's *capabilities* (`tools: *`, nesting depth, model frontmatter) instead of X's *contract* — if a plan says "X spawns Y" and cites only X's tool grants, the MAY question has not been asked. **Highest-risk shape:** the violation is **silent** — the callee is simply never invoked, every gate stays green, and an AC spot-check passes because the file exists. Severities: design viability resting on an **unread** binding constraint = `major` → **do NOT issue GO**; a cited-but-unverified constraint = `note`.
...
- **Blocker** — something that will panic at runtime, lose or corrupt persisted data, break a domain invariant, ship an unvalidated translation, or create unresolvable tech debt
