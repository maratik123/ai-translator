...
allowed-tools: Bash(cargo *) Bash(rustfmt *) Bash(git diff *) Bash(git rev-parse *) Bash(git checkout *) Bash(git branch *) Bash(git log *) Bash(git add *) Bash(git commit *) Bash(make *)
---
...
> Skill orchestrators must consult the AC-verification-grep re-run gate documented in `review-findings.md` § 0 and `self-review.md` § 2 — every design's "AC<N> verified by: <command>" line MUST be re-run against the shipped artefact before the verdict is finalised.
