...
#   This harness was transplanted from the lab-game project, so an inherited
#   citation points at that repository rather than at this one. Such a
#   citation MUST name its namespace -- the owning repository before the
#   number, the project name before a dated log entry, or a full path under
#   that project's memory namespace.
#
...
#   - already-namespace-qualified forms (naming lab-game)
...
while IFS=: read -r file line cite; do
  ...
  case "$txt" in *lab-game*) continue ;; esac
  ...
  #     plausible issue or pull-request number in either repository is six
  #     digits. Verified, not assumed: an earlier comment here claimed `\b`
...
# This repo's log starts 2026-09-17, the day the harness was transplanted. Any
# earlier date cannot be a local entry.
...
# fire.
rows2=$(scan '2026-(0[1-8]-[0-9]{2}|09-([01][0-6]))' .claude/ AGENTS.md ai-docs/) || scan_failed
...
while IFS=: read -r file line _; do
  ...
  echo "$txt" | grep -qiE 'lab-game' && continue   # already qualified
...
while IFS=: read -r file line _; do
  [ -z "$file" ] && continue
  txt=$(sed -n "${line}p" "$file")
  echo "$txt" | grep -qE 'projects/[^/[:space:]]*-lab-game/memory/' && continue
...
echo
if [ "$fail" -gt 0 ]; then
  echo "FAIL: ${fail} unresolvable citation(s). Each says 'here' but means another project."
  exit 1
fi
