...
- The canonical rustdoc contract-section order is `ai-docs/doc-convention.md § DOC-3 — Contract sections` (its sole authoritative home — AGENTS.md carries no duplicate order list). Confirm any file that restates the order agrees with DOC-3 exactly. The Review sync group (`self-review.md` § *Doc convention conformance*, `review-findings.md` § 6, `project-review/SKILL.md` step 6) is where restatements live; a restatement carrying another language's shapes — an exported-identifier rule, a name-first summary sentence, a package comment — is import residue, not a convention: this project is Rust, and DOC-1…DOC-6 as this repository spells them are the only sections.
...
**Worked example — the clash that rewrote this rule.** This project's design Subagent declared `name: design` while the harness ships an embedded `design` Skill. Nothing ever mis-resolved. The damage landed before any tool call was made:
...
**Why.** This harness was transplanted from another project of the same owner. A bare `#N` is repository-relative by GitHub convention; `learnings.md <date>` and a memory filename are project-relative. Imported text never changes on its own, but each referent silently rebinds from a real object in the source project to a nonexistent local one — and this repository's own logs started empty, so an inherited date citation pointed into nothing from the first day.

> **AXIOM — QUALIFY, never drop.** A repo-scoped probe cannot falsify a cross-repo reference: a bare `gh pr view` resolves against **this** repository and reports *Could not resolve* for a real, merged pull request of another one. That false negative reads as proof of fabrication and has already destroyed accurate provenance once. **Name the namespace, then re-probe.** Drop a citation only after `--repo`-qualified verification shows the referent genuinely does not exist.
...
Any hit → **`major`**. Fix by qualification: a bare `#N` that means another repository's pull request → `<owner>/<repo>#N`; a bare `` `ai-docs/learnings.md` `` date that means another project's log → that project's name before the path; a bare memory filename → its full `~/.claude/projects/<encoded>/memory/…` path.

**Do NOT replace the script with a substring hook.** It must tell a **citation** from an **example** (an illustrative `YYYY-MM-DD-slug.spec.md` filename asserts nothing) and a **use** from a **mention** (prose naming another project's pull request in order to retire its claim). A blanket matcher over-fires on both; the script's cue-word, field-name, and filename-shape filters exist for that reason, and it excludes `ai-docs/bugfix/**` (traces discuss citations by nature).
