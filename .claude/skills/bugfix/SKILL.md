...
allowed-tools: Bash(cargo *) Bash(rustfmt *) Bash(git rm ai-docs/bugfix/*) Bash(rm -f ai-docs/bugfix/*) Bash(make *)
---
...
3. State: `"Root cause: function_name() in crates/<crate>/src/<file>.rs line N — does X instead of Y"`
4. Append to the trace artifact:
 ```markdown
 ## Root Cause
 `function_name()` in `crates/<crate>/src/<file>.rs` line N — does X instead of Y
 Confirmed by user: ⏳ PENDING
 ```
...
- [ ] Test run: `cargo test --workspace -run TestName`
- [ ] Test is **RED** with the expected error (not compile error — specifically assertion fail)
- [ ] Only after a red test → Edit

**Test must:**
- Live in the file's own `#[cfg(test)]` module, or under the crate's `tests/` directory for a black-box test (`AGENTS.md` § *Workflow*; `ai-docs/rust-test-conventions.md`)
...
Agent(subagent_type="code-writer", prompt="
 ...
 cargo test --workspace <test name> — must turn GREEN
 cargo test --workspace — full suite; confirm nothing else broke
...
1. Run the failing test from Step 3: `cargo test --workspace -run TestName` — must turn green
2. Run the full suite: `cargo test --workspace` — confirm nothing else broke
3. Run `cargo clippy --workspace --all-targets` for changed files
4. Run `cargo fmt --all`
...
> ⛔ **`/bugfix` does not push until self-review issues APPROVE.** The fix is committed locally at Step 6 so that the reviewer has a range to read; the object of the AGENTS.md self-review AXIOM is the push. A `/bugfix` PR has the same code-quality bar as a `/task` PR — both land on master after merge. Build-system gates (lint / fmt / test) catch what the compiler and linters know about; they do NOT catch "this literal should be a named const", "this rustdoc paragraph contradicts the fix", "this fix touches a sibling concern that should be a separate PR" — exactly the class of nits a human reviewer raises.
