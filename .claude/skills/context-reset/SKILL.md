...
allowed-tools: Bash(cargo *) Bash(rustfmt *) Bash(git status) Bash(git rev-parse *) Bash(git branch *) Bash(git diff *) Bash(git add *) Bash(git commit *) Bash(make *)
---
...
1. `cargo build --workspace` — ensure code compiles
...
3. `cargo build --workspace` BEFORE handoff — don't pass broken code
