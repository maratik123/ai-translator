...
> **Dependabot is configured** (`.github/dependabot.yml`: `github-actions` and `cargo`, both weekly, each with its own commit prefix — `ci` and `build`). This skill triages the `cargo` PRs; a `github-actions` PR bails at preconditions as out of scope for v1.
...
- Handling non-Dependabot bot PRs (`renovate[bot]`, `pre-commit-ci[bot]`, …) or Dependabot **github-actions** PRs (v1 is cargo-only). Preconditions bail on these.
...
| Ecosystem is cargo | branch starts with `dependabot/github_actions/` (or any other non-cargo Dependabot ecosystem). v1 bails — out of scope. |
...
If the branch is `dependabot/github_actions/*` (or another non-cargo ecosystem), bail with the message: *"PR #<N> is a Dependabot `<ecosystem>` PR. `/dependabot-pr` v1 supports cargo only. Handle manually or wait for v2 ecosystem support."*
...
- **print-merge-command-and-pause.** Local `cargo build --workspace` validates the lockfile. On success, print the `gh pr merge --merge <N>` command (NEVER `--squash` / `--rebase`) and pause. The user runs the merge.
...
| Step 4 | Terminal action executed: `cargo build --workspace` clean (for confirm-merge) OR child `/pr-ci-failed` returned and verdict translated OR bail-comment / issue posted OR pause-for-user message printed |
