...
**Who authors a fix (binding).** A fix whose diff is predominantly `.rs` — with its `_test.rs`, `Cargo.toml` and migrations — is authored by `code-writer` in Mode B: the orchestrator plans it, the delegate authors it, runs its gates and returns without committing, and the orchestrator reads the diff and commits. A predominantly-prose fix is authored in-thread. Step 8's rule that the orchestrator never executes subtask code in its own context holds here too: `ai-docs/delegation-rules.md` § Phase 1 — Fit is the rule, and `ai-docs/learnings.md` 2026-09-13 records the `.rs` batch that was authored in-thread without it.
...
8. Commit `feat(<package>): <imperative summary>` with a 1–3 line body. No test counts in it: a figure recalled while the message is composed is not a measurement (`ai-docs/learnings.md` 2026-09-13).
