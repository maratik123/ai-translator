...
6. **Eval gates the commit (parent-thread ordering).** Run a targeted eval to verify each approved fix works; reports PASS/FAIL. The **parent thread** — which holds `Agent` — owns the ordering, and the ordering is **five stages, not three**: `classify → baseline batch → apply to the working tree → verification batch → gate on the PAIR`. **The baseline batch dispatches BEFORE anything is applied** — the pre-change state is obtained by not having applied anything yet, so an apply that precedes it is unrecoverable without restoring HEAD mid-flow. The `self-improve` subagent only *assembles* both reproducer blocks and yields; the parent holds them before it applies, which is the window the baseline slots into. **This sentence is a pointer, not the contract**: `ai-docs/improve-eval-contract.md` § *The RED baseline* is canonical, it outranks this restatement wherever they differ, and it is read **before the apply**, not when the restatement runs out — then **let the eval gate the commit** — a PASS proposal commits; a FAIL **reverts that proposal from the working tree before it lands** and loops back to Step 3 to strengthen the rule and re-run the eval. Both dispatches exclude the append-only logs, without which the baseline cannot go red on this corpus at all; and where every reproducer passes both ways **and** a **load-bearing** variant of one of them — the same clause reached as an incidental step inside a larger primary task, never as the whole question — also passes, the verdict is *out of instrument reach*, which commits only on an explicit owner decision recorded in the PR body. A re-run of the DEFAULT regime earns nothing: an earning condition whose extra dispatch varies no independent variable is the confirm-only gate under a label. This baseline-first, commit-gated-on-the-PAIR ordering is what keeps a failed proposal out of history — and what keeps a proposal the tree already satisfied from being recorded as a win it never earned. The reproducer template, the clean-context requirement, the forbidden degraded paths, and the assemble-vs-dispatch split live in `self-improve.md § Step 6` — this item references them and does **not** restate them.
