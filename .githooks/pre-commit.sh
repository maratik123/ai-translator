...
# script to judge it with. Each skip is named and loud — never a silent
# pass — because a gate that could not run is not a clean tree either.
staged=$(git diff --cached --name-only --diff-filter=ACMR 2>/dev/null | \
  grep -E '\.(rs|sh|sql|ya?ml)$|(^|/)(Makefile|\.gitignore)$|^\.githooks/')

gate="$root/ai-docs/scripts/check-comment-refs.sh"

if [ -z "$staged" ]; then
  printf 'pre-commit: no staged path of the comment-reference gated set; gate skipped\n' >&2
elif [ ! -f "$gate" ]; then
  printf 'pre-commit: no ai-docs/scripts/check-comment-refs.sh at the worktree root; comment-reference gate skipped\n' >&2
else
  bash "$gate" --staged
  status=$?
  if [ "$status" -ne 0 ]; then
