...
This applies with extra force to a delegate that also writes shared files. Both of these have proven false against the durable record in the project this harness came from: a `code-writer`'s *"I appended a learning"*, and its *"golden artefact verified by a checking subagent → PASS"*.
