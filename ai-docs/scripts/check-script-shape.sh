...
#   4. A tracked file that begins with a shebang and is not named with the
#      extension its interpreter pairs with, and is not a symbolic link to one
#      that is. Such a file is invisible to every tool here that finds scripts
#      by name — the lint target, the CI steps, and the reference gate's own
#      router. The pairing is the point, not the extension: a shell script
#      carries the shell extension, a Python script the Python one, and a file
#      whose name disagrees with its first line is findable by neither tool.
#
# A script that answers no help flag is not this checker's business. Nothing
# here says a script must have one. Checks 1 to 3 judge shell scripts, which is
# where the fixed dispatch shape is asserted; a gate written in Python answers
# the same flag, and its shape is reviewed rather than compared byte for byte.
#
...
# link's own name is what the extension rule is about. The wanted extension is
# read off the shebang itself, so the rule stays one rule as interpreters are
# added.
while IFS= read -r f; do
  [ -e "$f" ] || continue
  ...
  report "$f begins with a shebang for an interpreter whose extension is $want, and neither it nor its link target carries that extension"
done < <(git ls-files)
...
if [ "$findings" -gt 0 ]; then
  ...
  printf 'the extension its interpreter pairs with, or is a symbolic link to one\n' >&2
  printf 'that is.\n' >&2
  exit 1
fi
