...
# sandbox writes an untracked script at the path the dispatcher invokes; the
# stub records its argument vector in a marker file and takes its exit status
# from a fixture file, so both directions are drivable and the never-invoked
# case is observable. Untracked is load-bearing twice: nothing about the stub is
# ever staged, so the ratchet keeps taking its cheap exit, and the fixtures
# stage a shell probe, which the gate covers and the ratchet does not.
#
...
# Untracked, deliberately: nothing about the stub is ever staged, so the ratchet
# keeps taking its cheap exit, and the fixtures stage a shell probe, which the
# reference gate covers and the ratchet does not. The stub records its argument
# vector in a marker file and takes its exit status from a fixture file, so both
# directions are drivable and the never-invoked case is observable.
cat > "$gate/ai-docs/scripts/check-comment-refs.sh" <<'STUB'
#!/usr/bin/env bash
printf '%s\n' "$*" > gate-marker.txt
if [ "$(cat gate-verdict.txt 2>/dev/null)" = refuse ]; then
  echo "STUB-GATE-REFUSED" >&2
  exit 1
fi
exit 0
STUB
chmod +x "$gate/ai-docs/scripts/check-comment-refs.sh"

gate_commit() {   # gate_commit <label> <path-to-stage>
  ( cd "$gate" && git add -- "$2" >/dev/null 2>&1 && git commit -m "$1" 2>&1 )
}

# (a) the gate refuses: the commit is refused, and the stub's stderr reaches the
#     caller rather than being swallowed by the dispatcher.
...
# The invocation form itself, recorded by the stub rather than asserted from the
# dispatcher's source.
...
# (d) the gate script is gone: a named, loud skip, and the commit proceeds.
rm -f "$gate/ai-docs/scripts/check-comment-refs.sh"
rm -f "$gate/gate-marker.txt"
printf 'echo again\n' >> "$gate/probe.sh"
if ! out=$(gate_commit "no gate script" probe.sh); then
  printf 'FAIL [gate-skip-no-script]: the commit was refused although the skip is a pass\n%s\n' "$out"
  failures=$((failures + 1))
elif ! printf '%s' "$out" | grep -q 'comment-reference gate skipped'; then
  printf 'FAIL [gate-skip-no-script]: the skip was silent; it must name its reason\n%s\n' "$out"
  failures=$((failures + 1))
elif [ -e "$gate/gate-marker.txt" ]; then
  printf 'FAIL [gate-skip-no-script]: the gate ran with no gate script in the tree\n'
  failures=$((failures + 1))
fi
