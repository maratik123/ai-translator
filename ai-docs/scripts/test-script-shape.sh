...
expect_clean() {
  local name=$1 dir=$2 got out
  got=$(verdict "$dir")
  if [ "$got" != "PASS" ]; then
    out=$( cd "$dir" && bash "$checker" 2>&1 )
    printf 'FAIL [%s]: the checker refused a tree it must pass\n%s\n' "$name" "$out"
    failures=$((failures + 1))
  fi
}
...
# --- A shebang whose interpreter pairs with another extension ----------------
d=$(seed pyshebang)
printf '#!/usr/bin/env python3\nprint("ok")\n' > "$d/probe.py"
chmod +x "$d/probe.py"
git -C "$d" add probe.py
expect_clean "python-extension-pairs" "$d"

d=$(seed mismatched)
printf '#!/usr/bin/env python3\nprint("ok")\n' > "$d/probe.sh"
chmod +x "$d/probe.sh"
git -C "$d" add probe.sh
expect_flagged "interpreter-extension-mismatch" "$d" "probe.sh"
