...
if [ -z "$base" ]; then
  printf 'check-harness-gaps-forge: no merge base with master; provenance not checked\n' >&2
else
  # A fenced skeleton is a shape, not an entry — the shape half skips fences, and
  # so must this one, or the commit that first creates the log is refused for
  # carrying the very template the log documents. Content-addressed, because a
  # unified diff carries no fence state: a changed line whose text is one of the
  # current file's fenced lines is skeleton, not provenance.
  fenced=$(awk '/^```/ { f = !f; next } f' "$log" 2>/dev/null)
  changed=$(git diff -U0 "$base"...HEAD -- "$log" 2>/dev/null |
    grep -E '^[+-]\*\*(Forge|Closed by):\*\*' |
    while IFS= read -r line; do
      body=${line:1}
      printf '%s\n' "$fenced" | grep -qxF -- "$body" || printf '%s\n' "$line"
    done)
  if [ -n "$changed" ]; then
