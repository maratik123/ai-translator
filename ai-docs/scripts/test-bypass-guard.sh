...
body=$(jq -r '.hooks.PreToolUse[].hooks[].command
  | select(contains("Server-side protection of master belongs to the owner"))' "$settings")
...
r=repos/maratik123/ai-translator
...
check BLOCK "gh api --method=DELETE $r/branches/master/protection"
check BLOCK "gh api -X patch $r/rulesets/23358425 -f enforcement=disabled"
check BLOCK "gh api $r/rulesets --input ruleset.json"
check BLOCK "gh api $r/rulesets -f name=master -f enforcement=active"
check BLOCK "gh api $r/branches/master/protection -X PUT --input protection.json"
...
check ALLOW "gh api $r/rules/branches/master"
...
check ALLOW "gh api $r/branches/master/protection"
