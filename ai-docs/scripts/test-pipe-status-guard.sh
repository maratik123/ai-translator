...
BLOCK grep -n 'resultCh' crates/core/src/client.rs | cut -c1-150; echo $?
...
BLOCK cargo test --workspace 2>&1 | tee tmp/t.log; echo "exit=$?"
...
ALLOW grep -nw 'resultCh' crates/core/src/client.rs | cut -c1-150; echo "grep=${PIPESTATUS[0]} cut=${PIPESTATUS[1]}"
...
ALLOW cargo build --workspace > tmp/b.log 2>&1; echo "build=$?"
