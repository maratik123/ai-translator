...
check ALLOW 'non-reviewer spawn is out of scope' code-writer "$(cat <<'P'
  ...
  Group 2 of 3, subtasks 4-6. The migration lands first; run cargo test --workspace after each.
  Round 2 of the fix loop; cargo clippy was GREEN at 7efdbb7.
P
)"
...
check BLOCK 'recorded round-2 prompt (2026-09-03)' self-review "$(cat <<'P'
  ...
  Gates re-run after the last edit of the batch: cargo build GREEN, cargo clippy 0 issues, check-citations.sh PASS.
...
check BLOCK 'gate results alone' self-review "$(cat <<'P'
  ...
  Progress: ai-docs/plans/2026-09-04-name.progress.md
  7efdbb7..HEAD
  cargo build GREEN, cargo clippy 0 issues.
P
)"
