...
HIT	ac	| AC5 | A single `*slog.Logger` is constructed in the composition root and passed to every subsystem that accepts one, the migration call included; no shipped file under `crates/` obtains a logger from a package-level variable or from `slog.Default`. |
HIT	ac	| AC43 | Every exported item added by this task carries a doc comment beginning with its name, and every new crate carries a crate-level doc comment. |
HIT	ac	| AC48 | Module tidiness leaves `Cargo.toml` and `Cargo.lock` unchanged after the change. |
