...
check PASS names-a-test-file  'The package exports RunOnce, and crates/core/tests/worker.rs exists and passes.'
check PASS testdb-condition   'Every database-touching test in this change runs against a real PostgreSQL server through the shared test-database fixture, each in its own schema.'
...
check PASS word-boundary      'A tester may set DATABASE_URL; the attestation path is unaffected.'
check PASS plain-condition    'Crate core exists and carries a crate-level doc comment.'
