...
`AGENTS.md` § *Code Style* forbids discarding a `Result`.

**And the trial verdict survives that report — decided here rather than left to the implementor.** The
runner's conclusion is taken before the shutdown runs, and it is what the process exits with; a
teardown failure is written to the error channel and *raises* a green run to a failing exit status,
never *replaces* the status of a run the trials already failed. Concretely: no panicking call carries
the shutdown's result out of `main`, because a panic there substitutes its own status for the
runner's and costs the reader the trial verdict at exactly the moment the verdict matters most. The
signals are combined instead, the runner's first, so a red suite still reads as a red suite and a
leaked container still forbids an exit of zero
`[derived → the red observation "The container is removed after a failing trial" in § Test Design, which reads the process's exit status with a trial failed]`.
The admin pool is closed before the removal,
...
which this workspace does not enable: the measurement just made puts `properties-config` outside the
crate's default set, and the dev-dependency entry subtask 1 writes takes that default and adds
nothing to it
`[derived → subtask 1's contract]`
...
`[measured 435e649:docs/03-storage.md:4-8 · awk 'NR>=4 && NR<=8 {print NR": "$0}' docs/03-storage.md → the unticked task rows of § Задачи, ':7:' the testcontainers row naming ryuk, OnceCell and the sqlx test attribute, and ':8:' the vector-extension row]`
...
*Where the decision's ground is, and where it is not.* The series is the Postgres module's to fix
rather than this tree's: the module's published requirement is what admits the 0.27 line and nothing
newer, and that requirement is measured below, from the registry, on an artefact this task does not
write. What the tree then holds is the consequence — the dependency entry subtask 1 writes takes the
container crate from that series, and the lockfile it resolves records the container crate beside its
Postgres module
`[derived → subtask 1's contract, and the lockfile gate on its commit]`.
The lockfile corroborates the decision; it is not what decides it, and it is a file this task
rewrites, so it carries no coordinate here.
...
- **`one_container_serves_the_whole_binary`** — the process-wide start count, incremented at the site
  that awaits the container start, is asserted to be one; and two databases this trial takes for
  itself report the identical postmaster start instant, the server's own evidence that one server backs both.
