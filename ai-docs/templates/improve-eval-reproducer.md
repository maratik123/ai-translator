...
**R2 — the *recalled and misapplied* middle cell.** This is the one verdict a binary gate cannot express, so it gets an example of its own. The material is a recorded shape from the harness this one was ported from — the **same** requirement stated as **prose**, which was consciously invoked and still under-covered, versus stated as a **procedure**, which held. **Illustrative material, not an executed eval:** that work is merged, so nothing here was applied, dispatched, or graded.
