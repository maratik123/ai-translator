...
**at:** [commit SHA | `master`]    (REQUIRED whenever the entry contains a numeric claim — a count, a ratio, a byte size, a percentage; names the tree state the number was measured against. No `at:`, no number: a figure measured mid-task over a corpus the task itself is mutating describes nothing durable)
