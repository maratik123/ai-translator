...
| **out of instrument reach** (terminal) | every attempted reproducer passed both ways, **and** a **load-bearing** variant of at least one of them — the same clause reached as an *incidental step inside a larger primary task*, rather than as the whole question put to an idle agent — also passed | **Only** on explicit owner approval recorded in the PR body, with **all three** runs attached | None to the RULE — the remedy is to the **instrument**. File the reach failure in `ai-docs/harness-gaps.md`, naming the class it cannot reach |
