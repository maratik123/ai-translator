...
So the prior claim — *"structurally unfulfillable; the runtime tool exposure genuinely lacks `Agent`"* — is **false here**. **But it was true where it was written.** The project this harness descends from recorded it with evidence, after first falsifying the opposite hypothesis: the missing primitive was real, and the subagent genuinely could not fulfil the step itself.
...
Authority: a recorded pattern from the project this harness descends from — this Subagent silently degrading the eval step from clean-context evals to same-context close-reads. The rule stands on the clean-context requirement regardless of where it was first observed.
...
### Classes recorded out of instrument reach

A class that earns the *out of instrument reach* row is filed in `ai-docs/harness-gaps.md` and recorded **here as well**, because the gaps log is a forge's inbox while this page is what the next `/improve` pass reads — and that pass is the one which would otherwise spend eight dispatches re-learning what this one already measured. A row is written by the run that earned the cell, and removed by the run that reaches the class, naming the reproducer that did it.

| Class | Earned by | What the pass measured |
|---|---|---|
| **Probe discipline** — a probe's output is a fact about the probe until it has been shown to have measured its subject | the pass whose proposals landed in #73 | four framings across two proposals and eight clean-context dispatches — four baselines, two verifications, two load-bearing variants — and not one went red, so no proposal ever obtained the FAIL-before. Meanwhile the property recurred **three times inside that same run** — twice in dispatched agents, once in the orchestrating thread — every time while it was not the question being put |
| **Command text** — a pattern over command text matches text and never intent, and the issuing command line is inside the match space | the same pass | the same dispatches; the orchestrating thread reproduced it when a control string it wrote put the target literal back into the command line a bracket class was protecting |

**What a load-bearing variant must COST the dispatched agent, before its PASS is read as reach.** The two rows above were earned with load-bearing variants that passed, and a variant passes for two incompatible reasons: the agent honoured the clause under load, or the load never arrived. Only an artefact separates them. So the variant's SUBJECT block names a primary **deliverable** — something the grader reads, not a question that can be discharged in a sentence — and the returned answer carries that deliverable, finished, with the clause's occasion incidental to producing it. An answer that reaches the clause while the primary task is unstarted, truncated, or answered in one step is evidence about an undemanding prompt and not about the instrument, and the verdict falls back to *no valid reproducer*. This is the same test the row below states for the third run — **name the independent variable and check it is not already fixed** — applied to load, which is the variable a variant that costs the agent nothing quietly holds fixed.
...
| **out of instrument reach** (terminal) | every attempted reproducer passed both ways, **and** a **load-bearing** variant of at least one of them — the same clause reached as an *incidental step inside a larger primary task*, rather than as the whole question put to an idle agent — also passed | **Only** on explicit owner approval recorded in the PR body, with **all three** runs attached | None to the RULE — the remedy is to the **instrument**. File the reach failure in `ai-docs/harness-gaps.md`, naming the class it cannot reach, **and add its row to § *Classes recorded out of instrument reach*** so the next pass does not re-spend the dispatches |
...
**Precedent, inherited:** *"the reviewer builds its OWN representative input; a shared fixture buys correlated blindness, not economy"* — a validation recorded in the project this harness descends from. That entry extends the clean-context principle from *context* to *fixture*: an author cannot validate their own work with their own test input any more than in their own context. D1 is the same shape one level up — the author of the **rule** authoring the input that tests it.
