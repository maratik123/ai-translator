...
**KD-17 — Model weights and book texts are not in the repository.** They are local files, ignored by git. *Consequence:* a fixture that needs one either ships a small excerpt or is an eval that names what it ran against. *Source:* `AGENTS.md`, `.gitignore`.
