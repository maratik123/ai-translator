...
> | anything | **its own error channel and exit status, read apart from stdout** | a degradation notice **voids** a probe rather than decorating it; a wrong answer arrives on stdout with the complaint on stderr and a non-zero exit |
