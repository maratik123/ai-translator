...
**CRITICALLY**
1) **Language is split by surface, and the split is deliberate.** Russian: this file's project sections, `docs/`, `README.md`, commit messages and pull-request bodies — the repository's own history is Russian and stays that way. English: the harness (`.claude/**`, `ai-docs/**`), code, identifiers, comments, and the specs and designs under `ai-docs/plans/`. Conversation with the owner is Russian. **A literal that a harness file tells a flow to emit lands on the surface it is emitted to, never on the harness's own:** a commit-message or pull-request-body template quoted inside `.claude/**` is therefore written in Russian — its prose and its headings alike, while a locator it carries (`Closes #N`, `**Tracked in run:**`) is structure and stays as it is, and an English one instructs the violation in every run that follows it faithfully — after a push the only repair is a rewrite of pushed history, which § *Permissions* allows only on the owner's explicit approval.
...
> **Every cargo gate runs for real: there is no skip branch left in the recipes.** The workspace manifest sits at the root, so each recipe is the bare gate command, and a tree or a machine that cannot run it fails loudly instead of exiting 0 — a gate that could not run is not a green tree.

> **The test suite provisions its own database.** A database-backed test starts a `pgvector` container through testcontainers over the Podman socket and drops it on cleanup. `DATABASE_URL` is the application's connection, never the suite's — a test that reads it runs against the developer's own data. A machine with no container runtime is told so by the failing test, not by a silent skip. **The model client's conformance tests are built the same way** — they start `llama.cpp:server` **on the CPU, against the committed miniature models under `testdata/models/` and nothing else**, so a container runtime is not the database's business alone. The suite never talks to a model server that serves the application and never takes the GPU: the production model holds nearly all of the VRAM, and a test contending for it makes its own result and the server's throughput both unreliable. A check the miniature models cannot support is **raised with the owner**, never redirected at a real model.
...
> **AXIOM — a probe's output is a fact about the PROBE until you have shown it measured your subject. Before any number, coordinate or verdict from it is written down or reported, the report carries whichever artefacts below apply.**
>
> An instrument that never reached the subject, one configured for an adjacent question, and one that does not print the quantity being claimed all return output indistinguishable from a real measurement. The rule is not *be careful* — it is that the report carries an artefact a reader can check without re-running anything.
>
> | Before reporting... | The probe MUST print | The failure it rules out |
> |---|---|---|
> | anything | **that it landed on the subject** — the answering process or endpoint identified (`pgrep`, `ss`, a fresh port, the instance's own startup log), the file path, the region the tool actually judged | a stale process answers for the one you started |
> | anything | **its own error channel and exit status, read apart from stdout, and the preconditions the tool documents — a required sort order, an input encoding — confirmed before its verdict is read** | a degradation notice **voids** a probe rather than decorating it; a wrong answer arrives on stdout with the complaint on stderr and a non-zero exit |
> | a coordinate | **the coordinate itself** — `grep -n` for the clause's own words, or `awk 'NR>=a && NR<=b {print NR": "$0}'`. **NEVER** a range printer whose output you number by counting, and **NEVER** a range taken from the claim under test | the line number is inferred from a line's position in the output instead of read from the instrument |
> | a cost, a size, a count | **the operation's own accounting** — the served endpoint's own figure, the gate's own total. A helper endpoint answers an adjacent question with **different defaults**, and they are read before its output is quoted as the subject's figure | a proxy instrument's default silently changes what was measured |
>
> **If you cannot point at the line of output that carries the claim, you do not have the claim.** This binds every surface, not the search tools alone: a server probe, an HTTP measurement and a set difference are governed exactly as a `grep` is. [`.claude/rules/ast-index.md`](.claude/rules/ast-index.md) § *Positive results are NOT evidence either* is this AXIOM's search-surface instance, not its scope; § *Patterns* 2 states the same property in *Prefer* form over a subset of these shapes — this AXIOM is the binding statement, and the two are edited together.
...
>
> The tolerance starts at **0.00 pp**, and that is a starting value rather than a measurement: a tolerance is the width of the suite's own run-to-run drift, and no drift series has been run here yet — a crate carries tests now, so there is a drift to measure, and nobody has measured it. The first drift this project actually observes is what sets it, derived from a series of runs and never from a single blocked commit. The script's header carries the recipe.
...
- **Comments point at nothing outside themselves.** A comment says what the thing is, states its call contract, and carries what the toolchain requires; it carries no markdown path, design-section number, acceptance-criterion id, decision anchor, review-register finding id, issue number outside `TODO(#…)`, repository path, URL, or crate-qualified symbol of this workspace named outside its own crate. The reason is rot: the thing pointed at is edited and the comment becomes a claim nothing checks. `make comment-refs` gates the lexical half over the set the scanner's own language map decides — `*.rs`, `*.sh`, `*.bash`, `*.sql`, `*.yml`, `*.yaml`, `*.mk`, and any file named `Makefile` or `.gitignore` — by file name alone, with no directory anywhere in the rule: under `.githooks/` the `.sh` files are gated and the extensionless hook is not, which is why it is a symbolic link to `pre-commit.sh` rather than a script of its own. Narration and the bare unqualified name are review-judged. Full rule and exemptions: [`ai-docs/doc-convention.md`](ai-docs/doc-convention.md) § DOC-4.
...
- **A pattern over command TEXT matches text, never intent — and your own command line is inside the match space.** Two directions, both live here. *Describing* a matched form triggers the guard: the commit-block hook matches `git[[:space:]]+commit`, so a `gh issue create` / `gh pr create` / `gh pr comment` whose `--body` argument *contains* that substring is falsely blocked — use `--body-file <path>`, or the `Write` tool, rather than inlining such text into a shell command. *Issuing* a pattern puts the issuing process in range: `pkill -f <pattern>` matches every process whose full command line contains the pattern, **including the shell running the `pkill`** — kill by exact name (`pkill -x <name>`), by recorded PID, or with the flow's own stop control, and **NEVER** with a pattern the issuing command line itself contains. **A bracket class spares the issuing shell only while no other literal spelling of the target sits in the same command line** — a control string, an echo label or a comment written beside the probe puts the literal back into the corpus the probe is measuring. Most of this project's hook bodies state the first direction in their own refusal text, which is read only after the block — derive the set rather than trusting a tally here: `jq -r '.hooks[][].hooks[].command' .claude/settings.json | grep -c 'COMMAND TEXT'`. Treat the property as general rather than meeting it one guard at a time: a body that does not say it matches text still matches text.
...
*Prefer* treating a clean result from any verification apparatus — a control, a negative test, a baseline, a guard suite, a set intersection — as evidence about the **apparatus** first and the **subject** second. A genuinely absent effect and an instrument that cannot detect one look identical from the result alone, and only one of them is worth acting on. The binding form of this property is the probe AXIOM in § *Build & Test*, which requires rather than prefers, and which covers the shapes this enumeration leaves out — a coordinate read off a range printer, a cost quoted from a helper instrument. The two are edited together.
