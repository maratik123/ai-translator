...
- [ ] Тесты: `testcontainers` с образом `pgvector/pgvector:pg17` через Podman socket (`DOCKER_HOST=unix:///run/user/$UID/podman/podman.sock`), ryuk включен, контейнер один на тестовый бинарь (`OnceCell`); `#[sqlx::test]` создает БД на тест и накатывает миграции сам.
