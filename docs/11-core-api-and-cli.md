...
`eval` и `compare` запускаются с `temperature 0` (`top_k 1`), **выключенным MTP** и, по возможности, **в конфигурации без офлоада экспертов на CPU**. Причина в `05-llm-client.md`: при сэмплировании сервер недетерминирован даже с фиксированным seed (расхождение ~12%), а MTP меняет вывод ещё на ~24% из-за другой формы батча. Третье условие про офлоад — из замера бэкендов в `05-llm-client.md`: конфигурация с экспертами в host-памяти плавает на 20% от прогона к прогону, потому что зависит от состояния RAM и swap. Если офлоада не избежать, конфигурации в сравнении надо **перемежать и повторять**: эффект порядка здесь систематический, а не случайный, и последовательный прогон «по одной штуке» даёт ложные различия.

Без этих условий измеряется шум, а не эффект промпта.
...
- **доля нарушений глоссария** — в оригинале есть закреплённый термин, в переводе нет его эквивалента. Единственная из проверок, ловящая беглую правдоподобную бессмыслицу (разбор в `04-translation-pipeline.md`);
...
- **согласование внутри именной группы** — определение не согласовано с вершиной по роду, числу или падежу. Контекст не нужен, метрика дешёвая и ловит то, что остальные проверки пропускают;
- **доля смягчений** на абзацах с тяжёлым содержанием. Тихое смягчение проходит все текущие проверки `validate`, поэтому нужен отдельный прокси: детектор отказных формул на русском и на языке оригинала плюс обвал отношения длин оригинал/перевод;
...
Каждый прогон расцензуренной сборки сравнивается с немодифицированным контролем на той же базе (`ggml-org/gemma-4-26B-A4B-it-GGUF`): разница показывает, что именно расцензуривание дало и что забрало. Первая такая проверка описана в `05-llm-client.md` и закончилась отказом от расцензуренной сборки.
