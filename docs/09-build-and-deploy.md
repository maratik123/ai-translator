...
- [x] `testdata/models/`: мини-модели для тестов конформности клиента, закоммичены вместе с инструкцией происхождения и лицензиями. Раскладка внутри — та же, что в `models/`, то есть как на Hugging Face: `<organization>/<model_name>/<файлы>`. Продакшен-веса остаются в `models/` и в `.gitignore` — это разные вещи, и директории названы по-разному намеренно.
- [ ] Тесты конформности поднимают `ghcr.io/ggml-org/llama.cpp:server` в контейнере **на CPU и только на мини-моделях** из `testdata/models/`, и идут в обычном прогоне — локально и в CI. Значит контейнерный рантайм нужен не только Postgres. К серверу, обслуживающему приложение, набор тестов не обращается никогда: он держит GPU, и конкуренция за неё обесценивает и результат теста, и пропускную способность сервера.
